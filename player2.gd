...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("l_key") and !disable_pickup:
		if !carrying_player:
			...
			for player in players:
				if player != self and player.is_active:
					...
					if distance < 75:  # Increased from 50 to 75
						# Test if picking up would cause a collision
						if _can_safely_pickup(player):
							...
							print("Cannot pickup - not enough space")
						break
	...
	if carrying_player:
		...
		if Input.is_action_just_pressed("ui_up") and Input.is_action_pressed("l_key"):
